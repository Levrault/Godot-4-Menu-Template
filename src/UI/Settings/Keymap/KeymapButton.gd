...
func _ready() -> void:
	...
	connect("focus_entered",Callable(owner,"inner_navigation"),[self])
	connect("mouse_entered",Callable(owner,"inner_navigation"),[self])
...
func assign_with_constant(value: String) -> void:
	...
	if type == "keyboard":
		text = OS.get_keycode_string(EngineSettings.keylist[type][value])
		var input_event_key = InputEventKey.new()
		input_event_key.set_keycode(EngineSettings.keylist[type][value])
...
func clear() -> void:
	if owner.values.has(key):
		if EngineSettings.keylist["keyboard"].has(assigned_to):
			var input_event_key = InputEventKey.new()
			input_event_key.set_keycode(owner.values[key])
