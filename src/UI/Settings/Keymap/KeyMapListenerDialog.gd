...
func _ready():
	Events.connect("key_listening_started",Callable(self,"_on_Key_listening_started"))
	rebind_button.connect("pressed",Callable(self,"_on_Apply_pressed"))
	cancel_button.connect("pressed",Callable(self,"_on_Cancel_pressed"))
	timer.connect("timeout",Callable(self,"_on_Timer_timeout"))
	tick.connect("timeout",Callable(self,"_on_Tick_timeout"))
	tick.wait_time = timer.wait_time / progress_bar.max_value
	cancel_binding_message.text = tr("rebind.cancel_binding").format(
		{key = OS.get_keycode_string(InputMap.action_get_events("ui_cancel")[0].scancode)}
	)
...
func update_ui_for(step: int, data := {}):
	...
	if step == Step.remap:
		var key := OS.get_keycode_string(_current_event_identifier)
		var default_key := OS.get_keycode_string(_field.values.default)
		...
		unbind_message.text = tr("rebind.hold_to_unbind").format(
			{
				...
				key = OS.get_keycode_string(_current_event_identifier),
	...
	if step == Step.unbind:
		message.text = tr("rebind.unbinding_action").format(
			{key = OS.get_keycode_string(_current_event_identifier), action = _field.action}
		)
...
func map_keyboard_event(event: InputEventKey) -> void:
	...
	update_ui_for(Step.conflict, {key = OS.get_keycode_string(_new_event_identifier)})


func map_action() -> void:
	_button.assign_with_scancode(_new_event_identifier)
	_field._apply_changes(_button.key)
	close()
...
func _on_Apply_pressed() -> void:
	...
	_conflicted_field._apply_changes(_button.key)
	map_action()
