...
func _ready():
	await owner.ready
	InputManager.connect("device_changed",Callable(self,"_on_Device_changed"))
	Events.connect("locale_changed",Callable(self,"translate"))
	Events.connect("config_file_saved",Callable(self,"_on_Config_file_saved"))
	Events.connect("navigation_disabled",Callable(self,"set"),["disabled",true])
	connect("pressed",Callable(self,"_on_Pressed"))
	connect("mouse_entered",Callable(self,"_on_Mouse_entered"))

	confirmation_dialog.cancel_button.connect("pressed",Callable(self,"_on_Config_file_saved"))
	confirmation_dialog.confirm_button.connect("pressed",Callable(self,"_on_Confirmation_ok_pressed"))
	...
	for key in config_values:
		...
		if typeof(config_values[key]) == TYPE_DICTIONARY:
			...
			if config_sorted_values != engine_sorted_values:
				is_disabled = false
			continue
		if typeof(config_values[key]) == TYPE_FLOAT:
