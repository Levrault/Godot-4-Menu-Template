...
func _ready() -> void:
	...
	slider.connect("focus_exited",Callable(self,"emit_signal"),["field_focus_exited"])
	slider.connect("value_changed",Callable(self,"_on_Value_changed"))
	debounce_timer.connect("timeout",Callable(self,"_on_Timeout"))
...
func initialize() -> void:
	...
	# bad type
	if typeof(config_data) != TYPE_FLOAT and typeof(config_data) != TYPE_INT:
...
func reset() -> void:
	slider.disconnect("value_changed",Callable(self,"_on_Value_changed"))
