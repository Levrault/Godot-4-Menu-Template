...
# Node3D can be made with ui_left and ui_right
#
# e.g. engine.cfg
# ```
...
@export var placeholder := "placeholder" :
	get:
		return placeholder # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of _set_placeholder
...
func _ready() -> void:
	...
	connect("focus_entered",Callable(self,"emit_signal"),["field_focus_entered"])
	connect("focus_exited",Callable(self,"emit_signal"),["field_focus_exited"])
	previous.connect("pressed",Callable(self,"_on_Previous_value"))
	next.connect("pressed",Callable(self,"_on_Next_value"))
...
func _set_selected_key(text: String) -> void:
	super._set_selected_key(text)
	translate()


# TODO: Change to tweak file value
func play_arrow_animation(arrow: Button, to: Vector2) -> void:
...
func _on_Previous_value() -> void:
	...
	_tween.connect("tween_all_completed",Callable(self,"_on_Left_tween_completed"))
...
func _on_Next_value() -> void:
	...
	_tween.connect("tween_all_completed",Callable(self,"_on_Right_tween_completed"))
	play_arrow_animation(next, Vector2(next.rect_position.x + 5, next.rect_position.y))
	emit_signal("field_item_selected", selected_key)


func _on_Right_tween_completed() -> void:
	_tween.disconnect("tween_all_completed",Callable(self,"_on_Right_tween_completed"))
	play_arrow_animation(next, Vector2(next.rect_position.x - 5, next.rect_position.y))


func _on_Left_tween_completed() -> void:
	_tween.disconnect("tween_all_completed",Callable(self,"_on_Left_tween_completed"))
	play_arrow_animation(previous, Vector2(previous.rect_position.x - 5, previous.rect_position.y))
