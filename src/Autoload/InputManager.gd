...
var device: String = default_gamepad :
	get:
...
# Utils function to quickly remove_at a motion event
...
# Utils function to quickly remove_at a motion event with axis_value
...
# Utils function to remove_at a joy button event
	...
	for evt in InputMap.action_get_events(action):
		if for_device == DEVICE_KEYBOARD or for_device == DEVICE_MOUSE:
			if evt is InputEventKey:
				return OS.get_keycode_string(evt.scancode)
		else:
